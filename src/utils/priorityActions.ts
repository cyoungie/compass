...
/** All priorities (no limit) for the Priorities list screen. */
export function getFullPriorityActions(profile: UserProfile): PriorityAction[] {
  const actions: PriorityAction[] = [];
  ...
  if (profile.legal_gaps && profile.legal_gaps.length > 0) {
    actions.push({
      id: 'legal',
      tag: 'opportunity',
      title: 'Address legal gaps',
  ...
  if (profile.housing_status?.toLowerCase().includes('unstable')) {
    actions.push({
      id: 'housing',
      tag: 'opportunity',
      title: 'Explore housing options',
